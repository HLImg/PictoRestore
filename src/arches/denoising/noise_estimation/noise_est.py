...
from src.arches.denoising.spatial_channel.transformer import PSCT
...
class ResidualGroup(nn.Module):
    def __init__(self, 
        ...
        for index in range(num_blk):
            module_body.append(
                PSCT(in_dim, head_dim_s=head_dim_s, head_num_c=head_num_c, 
        ...
        for i in range(len(num_group)):
            ...
            module_body.append(
                ResidualGroup(num_blk=num_blk, in_dim=num_feats, head_dim_s=head_dim_s,
                              ...
                              bias=bias, pool=pool, pool_mode=pool_mode)
            )
